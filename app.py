...
from utils.image_processing import (
    ...
    split_selected_color_masks,
)
from utils.peak_detection import detect_peaks
from utils.report import build_report
from agents.spectral_agent import get_environment_ollama_host, list_installed_models, run_spectral_agent
...
APP_VERSION = "2.2 — Ollama local + múltiplas cores"
AI_BACKEND_ID = "ollama-local-v2-multicolor"
APP_SUBTITLE = "Extração, reconstrução e análise de espectros com interpretação local pelo Ollama."
...
def make_multi_line_plot(long_df: pd.DataFrame, x_label: str, y_label: str, title: str):
    """Plot multiple curves while preserving each curve's stored hexadecimal color."""
    color_map: dict[str, str] = {}
    if {"curve", "color_hex"}.issubset(long_df.columns):
        for curve_name, group in long_df.dropna(subset=["color_hex"]).groupby("curve"):
            value = str(group["color_hex"].iloc[0])
            if value.startswith("#") and len(value) == 7:
                color_map[str(curve_name)] = value
    fig = px.line(
        long_df,
        x="x",
        y="y",
        ...
        title=title,
        labels={"x": x_label, "y": y_label, "curve": "Curva"},
        hover_data=[c for c in ["color_label", "color_hex", "color_rgb"] if c in long_df.columns],
    )
...
def curves_dict_to_long_df(
    curves: dict[str, pd.DataFrame],
    metadata: dict[str, dict] | None = None,
) -> pd.DataFrame:
    """Convert curves to long format and repeat color metadata on every row."""
    frames = []
    ...
    for name, df in curves.items():
        ...
        tmp["color_label"] = info.get("color_label")
        tmp["color_hex"] = info.get("hex")
        rgb = info.get("rgb")
        tmp["color_rgb"] = None if rgb is None else ",".join(str(int(v)) for v in rgb)
        frames.append(tmp)
    if not frames:
        return pd.DataFrame(columns=["curve", "color_label", "color_hex", "color_rgb", "x", "y", "pixel_x", "pixel_y"])
    return pd.concat(frames, ignore_index=True)


def curve_metadata_from_items(items: list[dict]) -> dict[str, dict]:
    """Build a JSON-safe catalog without embedding binary masks."""
    ...
    for ordinal, item in enumerate(items, start=1):
        ...
        catalog[name] = {
            ...
            "rgb": item.get("rgb"),
            "hex": item.get("hex"),
            "detected_rgb": item.get("detected_rgb"),
            "detected_hex": item.get("detected_hex"),
            "selection_mode": item.get("selection_mode"),
            "mask_pixels": int(item.get("area", 0) or 0),
        }
    return catalog
...
def reset_ai_analysis_state(clear_history: bool = True) -> None:
    """Invalidate LLM outputs when the underlying curve or peak data changes."""
    st.session_state.diagnostic = ""
    st.session_state.agent_tool_trace = []
...
with st.sidebar:
    ...
    st.caption(f"{APP_VERSION} · processamento determinístico + modelo local")
    st.divider()
...
4. Calibre os eixos  
5. Extraia a curva e detecte picos  
6. Consulte o assistente com tools locais  
"""
    )
    st.divider()
    ...
    ollama_host = st.text_input(
        "Servidor Ollama",
        value=get_environment_ollama_host(),
        help="Por padrão, o Ollama local atende em http://localhost:11434.",
    ).strip()

    try:
        installed_models = list_installed_models(ollama_host)
        st.success(f"Ollama conectado · {len(installed_models)} modelo(s) instalado(s).")
    except Exception as exc:
        installed_models = []
        st.warning(str(exc))

    recommended_models = ["qwen3:1.7b", "qwen3:0.6b", "qwen3:4b"]
    model_options = installed_models or recommended_models
    if "qwen3:1.7b" in model_options:
        default_model_index = model_options.index("qwen3:1.7b")
    else:
    ...
    ollama_model = st.selectbox(
        ...
        help="qwen3:1.7b é o padrão leve. Instale modelos com: ollama pull nome-do-modelo",
    )
    ollama_temperature = st.slider(
        "Temperatura", 0.0, 1.0, 0.2, 0.1,
        help="Valores baixos favorecem consistência em análise técnico-científica.",
    )
    ollama_top_p = st.slider(
        "Top-p", 0.1, 1.0, 0.9, 0.05,
        help="Controla a massa de probabilidade usada na amostragem. Em experimentos, altere temperatura ou top-p, não ambos ao mesmo tempo.",
    )
    ollama_thinking = st.checkbox(
        "Ativar thinking do modelo",
        value=False,
        help="Pode melhorar tarefas complexas, mas aumenta latência. O conteúdo de raciocínio não é exibido.",
    )
    ollama_max_tokens = st.slider("Máximo de tokens gerados", 300, 2000, 1000, 100)
    ollama_context_window = st.select_slider(
        "Janela de contexto",
        options=[2048, 4096, 8192, 16384],
        value=4096,
        help="Contextos maiores consomem mais memória. 4096 é adequado ao fluxo atual e a hardware limitado.",
    )
    st.divider()
    st.info("Para melhores resultados, use imagens com fundo claro, curvas bem separadas e eixos calibrados.")

st.title(APP_TITLE)
st.write(APP_SUBTITLE)
st.caption(f"Versão ativa: **{APP_VERSION}** · backend: `Ollama local` · nenhuma chave de API é necessária.")
...
# Limpa respostas antigas quando a aplicação muda de provedor/versão.
# Isso evita exibir conteúdo remanescente de uma execução anterior na mesma sessão do navegador.
if st.session_state.get("_ai_backend_id") != AI_BACKEND_ID:
    st.session_state.diagnostic = ""
    st.session_state.agent_tool_trace = []
    st.session_state.agent_usage = {}
    st.session_state.chat_history = []
    st.session_state._ai_backend_id = AI_BACKEND_ID
...
A aplicação integra um **modelo local executado pelo Ollama** para interpretar os resultados. O modelo não calcula os dados diretamente: ele consulta ferramentas determinísticas que retornam estatísticas, picos, comparação de curvas e qualidade da extração.
"""
    )
...
- Exportação CSV
- Assistente local Ollama com tool calling
...
"""
        ...
        multi_color_strategy = "automatic"
        selected_color_specs: list[dict] = []
        manual_color_tolerance = 28.0
        if extraction_scope == "multi_color":
            multi_color_strategy = st.radio(
                "Como definir as cores das curvas?",
                ["manual", "automatic"],
                horizontal=True,
                format_func=lambda value: {
                    "manual": "Selecionar várias cores manualmente",
                    "automatic": "Detectar cores automaticamente",
                }[value],
                help=(
                    "A seleção manual é mais controlável: cada cor gera uma máscara independente e não sobreposta. "
                    "A detecção automática é útil quando as cores ainda não são conhecidas."
                ),
            )
            if multi_color_strategy == "manual":
                n_selected_colors = st.slider(
                    "Número de cores/curvas selecionadas",
                    min_value=2,
                    max_value=10,
                    value=min(max_curves, 3),
                )
                manual_color_tolerance = float(
                    st.slider(
                        "Tolerância perceptual das cores (CIELAB)",
                        min_value=5,
                        max_value=80,
                        value=28,
                        help=(
                            "Valores menores exigem uma cor muito próxima da escolhida. Valores maiores aceitam "
                            "antialiasing e variações de brilho, mas podem incluir elementos semelhantes."
                        ),
                    )
                )
                defaults = [
                    ("Curva azul", "#1F77B4"),
                    ("Curva vermelha", "#D62728"),
                    ("Curva verde", "#2CA02C"),
                    ("Curva roxa", "#9467BD"),
                    ("Curva laranja", "#FF7F0E"),
                    ("Curva ciano", "#17BECF"),
                    ("Curva rosa", "#E377C2"),
                    ("Curva marrom", "#8C564B"),
                    ("Curva preta", "#111111"),
                    ("Curva amarela", "#BCBD22"),
                ]
                st.markdown("**Cores que devem ser rastreadas**")
                for color_index in range(n_selected_colors):
                    name_default, hex_default = defaults[color_index]
                    name_col, picker_col, data_col = st.columns([2.2, 1, 1.5])
                    selected_name = name_col.text_input(
                        f"Nome da curva {color_index + 1}",
                        value=name_default,
                        key=f"manual_curve_name_{color_index}",
                    ).strip()
                    selected_hex = picker_col.color_picker(
                        f"Cor {color_index + 1}",
                        value=hex_default,
                        key=f"manual_curve_color_{color_index}",
                    ).upper()
                    selected_rgb = hex_to_rgb(selected_hex)
                    data_col.code(f"RGB {selected_rgb}\n{selected_hex}", language=None)
                    selected_color_specs.append({
                        "name": selected_name or f"Curva {color_index + 1}",
                        "rgb": selected_rgb,
                        "hex": selected_hex,
                    })
                st.caption(
                    "Cada cor selecionada produzirá sua própria máscara. Um pixel será atribuído somente à cor "
                    "selecionada mais próxima, evitando sobreposição entre as curvas."
                )

        extraction_method = "auto_color"
        threshold = 130
        color_name = "custom"
        color_tolerance = int(manual_color_tolerance)
        custom_rgb = (0, 0, 255)

        if extraction_scope == "multi_color":
            if multi_color_strategy == "automatic":
                color_tolerance = st.slider(
                    "Sensibilidade da detecção automática de cores",
                    5,
                    180,
                    90,
                    help="Controla a saturação mínima usada para separar grupos de cores automaticamente.",
                )
        else:
            c1, c2, c3, c4 = st.columns(4)
            extraction_method = c1.selectbox(
                ...
                help="Use 'Automático para curva colorida' quando a curva for colorida e a grade/eixos forem cinza ou preto.",
            )
            threshold = c2.slider(
                "Threshold",
                0,
                255,
                130,
                help="Usado principalmente no modo Contraste.",
            )
            color_name = c3.selectbox(
                "Cor da curva",
                ["blue", "red", "green", "black", "custom"],
                format_func=lambda x: {
                    "blue": "Azul",
                    "red": "Vermelho",
                    "green": "Verde",
            ...
            color_tolerance = c4.slider(
                "Tolerância/saturação",
                5,
                180,
                90,
                help="Em HSV/RGB controla a tolerância de cor; no automático controla a saturação mínima.",
            )
            if color_name == "custom":
                color_hex = st.color_picker("Cor personalizada", "#0000ff")
                custom_rgb = hex_to_rgb(color_hex)
        ...
        if run_extraction:
            try:
                if extraction_scope == "multi_color":
                    if multi_color_strategy == "manual":
                        curve_masks_info = split_selected_color_masks(
                            source_rgb,
                            color_specs=selected_color_specs,
                            tolerance=manual_color_tolerance,
                        ...
                        missing_names = [
                            ...
                            if spec["name"] not in {item["name"] for item in curve_masks_info}
                        ]
                        if missing_names:
                            st.warning(
                                "Não foram encontrados pixels suficientes para: " + ", ".join(missing_names) + ". "
                                "Aumente a tolerância perceptual ou confira as cores selecionadas."
                            )
                    else:
                    ...
                    if not curve_masks_info:
                        raise ValueError(
                            "Nenhuma curva colorida foi detectada. Confira as cores escolhidas, aumente a "
                            "tolerância, recorte melhor a imagem ou use a detecção automática."
                        )
                    masks = [item["mask"] for item in curve_masks_info]
                    overlay_colors = [item.get("rgb") for item in curve_masks_info]
                    overlay = overlay_multiple_masks(source_rgb, masks, colors=overlay_colors)
                    curve_metadata = curve_metadata_from_items(curve_masks_info)
                    ...
                    for item in curve_masks_info:
                        ...
                        if curve_df_i is None or curve_df_i.empty:
                            continue
                    ...
                    if not curves:
                        raise ValueError("As máscaras foram criadas, mas nenhuma curva pôde ser rastreada.")
                    curve_metadata = {name: curve_metadata[name] for name in curves if name in curve_metadata}
                    long_df = curves_dict_to_long_df(curves, curve_metadata)
                    ...
                    st.session_state.curve_metadata = curve_metadata
                    st.session_state.quality = combined_quality
                    st.session_state.peaks_df = pd.DataFrame()
                    strategy_label = "cores selecionadas" if multi_color_strategy == "manual" else "cores detectadas automaticamente"
                    st.success(f"{len(curves)} curva(s) extraída(s) com {strategy_label}.")
                    ...
                    for item in curve_masks_info:
                        color_info = describe_mask_color(source_rgb, item["mask"])
                        item.update(color_info)
                        item["detected_rgb"] = color_info["rgb"]
                        item["detected_hex"] = color_info["hex"]
                        item["name"] = f"{item['name']} ({color_info['color_label']})"
                    masks = [item["mask"] for item in curve_masks_info]
                    overlay = overlay_multiple_masks(source_rgb, masks, colors=[item.get("rgb") for item in curve_masks_info])
                    curve_metadata = curve_metadata_from_items(curve_masks_info)
                    ...
                    single_color = describe_mask_color(source_rgb, mask)
                    st.session_state.multi_curves = {}
                    st.session_state.multi_curve_long_df = pd.DataFrame()
                    st.session_state.multi_curve_mode = False
                    st.session_state.curve_metadata = {
                        "Curva única": {
                            "ordinal": 1,
                            "name": "Curva única",
                            "color_label": single_color["color_label"],
                            "rgb": single_color["rgb"],
                            "hex": single_color["hex"],
                            "detected_rgb": single_color["rgb"],
                            "detected_hex": single_color["hex"],
                ...
                reset_ai_analysis_state(clear_history=True)
                st.session_state.extraction_settings = {
                    ...
                    "Estratégia de cores": multi_color_strategy if extraction_scope == "multi_color" else None,
                    "Cores selecionadas": [
                        {"name": spec["name"], "rgb": list(spec["rgb"]), "hex": spec["hex"]}
                        for spec in selected_color_specs
                    ] if extraction_scope == "multi_color" and multi_color_strategy == "manual" else [],
                    "Tolerância CIELAB": manual_color_tolerance if extraction_scope == "multi_color" and multi_color_strategy == "manual" else None,
...
with tabs[4]:
    ...
    curve_metadata = st.session_state.get("curve_metadata", {})
    ...
    if curve_df is None or curve_df.empty:
        ...
        if multi_mode and multi_curves:
            ...
            if curve_metadata:
                st.subheader("Curvas e cores associadas")
                catalog_rows = []
                for name, info in curve_metadata.items():
                    rgb = info.get("rgb")
                    catalog_rows.append({
                        "Ordem": info.get("ordinal"),
                        "Curva": name,
                        "Cor": info.get("color_label"),
                        "RGB": None if rgb is None else tuple(rgb),
                        "HEX": info.get("hex"),
                        "Modo": info.get("selection_mode"),
                        "Pixels na máscara": info.get("mask_pixels"),
                    })
            ...
            if table_mode == "long":
                export_columns = [
                    column
                    for column in ["curve", "color_label", "color_rgb", "color_hex", "x", "y", "pixel_x", "pixel_y"]
                    if column in long_df.columns
                ]
                st.dataframe(long_df[export_columns], use_container_width=True)
                st.download_button(
                    "Baixar dados de todas as curvas em CSV",
                    data=dataframe_to_csv_bytes(long_df[export_columns]),
...
with tabs[6]:
    st.header("Assistente SpectraCurve — Ollama local")
    curve_df = st.session_state.curve_df
    ...
    if curve_df is None or curve_df.empty:
        ...
        multi_mode = bool(st.session_state.get("multi_curve_mode", False))
        multi_curves = st.session_state.get("multi_curves", {})
        curves_for_agent = multi_curves if multi_mode and multi_curves else {"Curva única": curve_df}
        quality_obj = st.session_state.get("quality")
        quality_dict = (
            {
                "n_pixels": quality_obj.n_pixels,
                "n_columns_with_curve": quality_obj.n_columns_with_curve,
                "coverage_fraction": quality_obj.coverage_fraction,
                "estimated_quality": quality_obj.estimated_quality,
                "message": quality_obj.message,
            }
            if quality_obj
            else {}
        )
        agent_context = SpectralContext(
            spectrum_type=st.session_state.get("spectrum_type", "Absorção / UV-Vis"),
            curves=curves_for_agent,
        ...
        st.caption(
            f"Modelo: `{ollama_model}` · temperatura: `{ollama_temperature:.1f}` · "
            f"top-p: `{ollama_top_p:.2f}` · thinking: `{ollama_thinking}`"
        )
        st.info(
            "Os valores numéricos vêm das ferramentas Python. O modelo é responsável pela interpretação e pela redação, não pelos cálculos."
        )

        quick_col1, quick_col2, quick_col3, quick_col4 = st.columns(4)
        quick_question = None
        if quick_col1.button("Listar curvas e cores"):
            quick_question = "Liste todas as curvas reconhecidas, preservando a ordem, o nome, o rótulo da cor, o RGB e o código HEX."
        if quick_col2.button("Gerar diagnóstico técnico"):
            quick_question = "Faça um diagnóstico técnico do espectro, destacando resultados numéricos, interpretação cautelosa e limitações."
        if quick_col3.button("Interpretar picos/bandas"):
            quick_question = "Analise os picos ou bandas já detectados. Identifique os mais relevantes sem inventar atribuições químicas."
        if quick_col4.button("Avaliar qualidade"):
            quick_question = "Avalie a qualidade da extração e recomende ajustes objetivos quando necessários."

        user_question = st.text_area(
            "Pergunta para o assistente",
            value="",
            placeholder="Ex.: Quais são os principais resultados e quais conclusões não podem ser feitas com estes dados?",
            height=100,
        )
        run_question = quick_question
        if st.button("Enviar pergunta ao modelo", type="primary"):
            run_question = user_question.strip()

        if run_question is not None:
            if not run_question.strip():
                st.warning("Digite uma pergunta antes de enviar.")
            else:
                try:
                    with st.spinner("Consultando o modelo local e executando as ferramentas necessárias..."):
                    ...
                    st.session_state.diagnostic = result.text
                    st.session_state.agent_tool_trace = result.tool_trace
                    st.session_state.agent_usage = result.usage
                    st.session_state.chat_history.append({"question": run_question, "answer": result.text})
                except Exception as exc:
                    st.error(str(exc))
        ...
        if st.session_state.get("diagnostic"):
            ...
            usage = st.session_state.get("agent_usage", {})
            if usage:
                u1, u2, u3 = st.columns(3)
                u1.metric("Tokens do prompt", usage.get("prompt_tokens") or "—")
                u2.metric("Tokens gerados", usage.get("output_tokens") or "—")
                u3.metric("Tempo total", f"{usage.get('duration_seconds'):.2f} s" if usage.get("duration_seconds") else "—")

            trace = st.session_state.get("agent_tool_trace", [])
            with st.expander(f"Rastreamento das tools ({len(trace)} chamada(s))"):
                if trace:
                    for item in trace:
                        origin = item.get("invoked_by", "model")
                        st.markdown(f"**Rodada {item['round']} — `{item['tool']}` · origem: `{origin}`**")
                        st.json({"argumentos": item["arguments"], "resultado": item["output"]})
                else:
                    st.caption("Nenhuma tool registrada para a última resposta.")

        if st.session_state.get("chat_history"):
            with st.expander("Histórico da sessão"):
                for idx, item in enumerate(reversed(st.session_state.chat_history[-5:]), start=1):
                    st.markdown(f"**Pergunta {idx}:** {item['question']}")
...
with tabs[7]:
    ...
    if curve_df is None or curve_df.empty:
        ...
        diagnostic = st.session_state.get("diagnostic") or (
            "O diagnóstico com IA ainda não foi gerado. Use a aba Assistente Ollama para produzir uma interpretação fundamentada nas ferramentas locais."
        )
        ...
        report = build_report(
            ...
            curve_catalog=st.session_state.get("curve_metadata", {}),
        )
        ...
        report_data_df = (
            st.session_state.get("multi_curve_long_df", pd.DataFrame())
            if st.session_state.get("multi_curve_mode", False)
            else curve_df[["x", "y"]]
        )
        c2.download_button(
            "Baixar dados CSV",
            data=dataframe_to_csv_bytes(report_data_df),
            file_name=(
                "spectracurve_multi_curves.csv"
                if st.session_state.get("multi_curve_mode", False)
                else "spectracurve_extracted_data.csv"
            ),
            mime="text/csv",
        )
