...
def build_report(
    ...
    lines.append("3. Resumo numérico")
    lines.append("-" * 20)
    for key, value in curve_summary.items():
        lines.append(f"{key}: {value}")
    lines.append("")

    lines.append("4. Picos/bandas detectados")
    lines.append("-" * 28)
    ...
    lines.append("5. Diagnóstico simulado")
    lines.append("-" * 24)
    ...
    lines.append("A interpretação é simulada e não utiliza modelo generativo nesta versão.")
