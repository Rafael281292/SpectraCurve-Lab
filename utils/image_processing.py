...
def rgb_to_hex(rgb: tuple[int, int, int] | list[int]) -> str:
    """Convert an RGB triplet to a normalized hexadecimal color."""
    r, g, b = (max(0, min(255, int(v))) for v in rgb)
    return f"#{r:02X}{g:02X}{b:02X}"


def hex_to_rgb(value: str) -> tuple[int, int, int]:
    """Convert #RRGGBB to an RGB triplet."""
    ...
    try:
        ...
        raise ValueError(f"Cor hexadecimal inválida: {value}") from exc


def describe_rgb_color(rgb: tuple[int, int, int] | list[int]) -> str:
    """Return an approximate Portuguese color label for an RGB triplet."""
    arr = np.uint8([[list(rgb)]])
    hsv = cv2.cvtColor(arr, cv2.COLOR_RGB2HSV)[0, 0]
    hue, saturation, value = (int(v) for v in hsv)
    if value <= 45:
        return "preta"
    if saturation <= 30:
        if value >= 220:
        ...
        if value >= 120:
            return "cinza"
        return "cinza escura"
    return _hue_to_label(float(hue))


def describe_mask_color(rgb: np.ndarray, mask: np.ndarray) -> dict:
    """Summarize the representative color of the selected pixels."""
    selected = rgb[mask.astype(bool)]
    if selected.size == 0:
        representative = (0, 0, 0)
    else:
        representative = tuple(int(round(v)) for v in np.median(selected, axis=0))
    return {
        "color_label": describe_rgb_color(representative),
        "rgb": list(representative),
        "hex": rgb_to_hex(representative),
    }


def split_selected_color_masks(
    rgb: np.ndarray,
    ...
    remove_grid: bool = True,
    remove_axes: bool = True,
    max_components_per_curve: int = 10,
) -> list[dict]:
    """Create one non-overlapping mask for each color explicitly selected by the user.

    Pixels are assigned to the nearest selected color in CIELAB space. This is more robust
    than raw RGB distance for antialiasing and brightness variation, and the nearest-color
    assignment prevents the same pixel from belonging to two selected curves.
    """
    if rgb.ndim != 3 or rgb.shape[2] != 3:
        raise ValueError("Expected RGB image with shape (height, width, 3).")
    if not color_specs:
        raise ValueError("Selecione pelo menos uma cor para extrair as curvas.")

    normalized_specs: list[dict] = []
    used_names: set[str] = set()
    for index, spec in enumerate(color_specs, start=1):
        raw_rgb = spec.get("rgb")
        if raw_rgb is None and spec.get("hex"):
            raw_rgb = hex_to_rgb(spec["hex"])
        if raw_rgb is None or len(raw_rgb) != 3:
            raise ValueError(f"A seleção de cor {index} não possui RGB válido.")
        target_rgb = tuple(max(0, min(255, int(v))) for v in raw_rgb)
        color_label = str(spec.get("color_label") or describe_rgb_color(target_rgb)).strip()
        base_name = str(spec.get("name") or f"Curva {index} ({color_label})").strip()
        if not base_name:
            base_name = f"Curva {index} ({color_label})"
        name = base_name
        suffix = 2
    ...
    target_rgb_array = np.uint8([[list(spec["rgb"]) for spec in normalized_specs]])
    target_lab = cv2.cvtColor(target_rgb_array, cv2.COLOR_RGB2LAB)[0].astype(np.float32)
    distances = np.stack(
        [np.linalg.norm(source_lab - target.reshape(1, 1, 3), axis=2) for target in target_lab],
        axis=2,
    )
    closest_index = np.argmin(distances, axis=2)
    closest_distance = np.min(distances, axis=2)
    candidate = closest_distance <= float(tolerance)

    results: list[dict] = []
    for index, spec in enumerate(normalized_specs):
        mask = candidate & (closest_index == index)
        mask = _remove_border_pixels(mask, border_px=2)
        if remove_axes:
            mask = _remove_long_straight_lines(mask, row_fraction=0.42, col_fraction=0.42)
        if remove_grid:
        ...
        mask = _remove_small_components(mask, min_component_area=max(1, int(min_component_area)))
        mask = keep_relevant_components(
            mask,
            max_components=max(1, int(max_components_per_curve)),
            min_width_fraction=0.015,
        )
        if not mask.any():
            continue

        detected = describe_mask_color(rgb, mask)
        results.append({
            "name": spec["name"],
            "color_label": spec["color_label"],
            "rgb": list(spec["rgb"]),
            "hex": spec["hex"],
            "detected_rgb": detected["rgb"],
            "detected_hex": detected["hex"],
            "area": int(mask.sum()),
            "components": None,
...
def split_colored_curve_masks(
    ...
    for i, group in enumerate(groups[: int(max_curves)]):
        ...
        color_info = describe_mask_color(rgb, curve_mask)
        out.append({
            ...
            "rgb": color_info["rgb"],
            "hex": color_info["hex"],
            "detected_rgb": color_info["rgb"],
            "detected_hex": color_info["hex"],
...
def overlay_multiple_masks(
    ...
    colors: list[tuple[int, int, int] | list[int] | None] | None = None,
) -> np.ndarray:
    """Overlay multiple masks with their stored colors or a validation palette."""
    overlay = rgb.copy()
    palette = np.array([
        [255, 0, 0],
        [0, 120, 255],
    ...
    for i, mask in enumerate(masks):
        if mask.shape[:2] != rgb.shape[:2]:
            raise ValueError("Mask and image must have the same height and width.")
        chosen = None if colors is None or i >= len(colors) else colors[i]
        if chosen is None:
            paint = palette[i % len(palette)]
        else:
            paint = np.asarray(chosen, dtype=np.uint8)
        overlay[mask.astype(bool)] = paint
    return overlay
